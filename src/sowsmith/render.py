"""SOWSmith renderer — deterministic Statement of Work from an envelope.

Consumes a parser-os ``orbitbrief.input.v2`` envelope (dict) and emits
a contract-grade Statement of Work in markdown. Every claim traces to
a specific atom ID; missing-fields surface as ``[NEEDS DATA: field_id]``
so a PM sees gaps instead of silently shipping incomplete contracts.

Zero runtime dependencies on parser-os. The envelope JSON contract
(``orbitbrief.input.v2``) is the only coupling — pass a dict, get
markdown back. CLI usage::

    sowsmith render path/to/orbitbrief.input.json --out sow.md

Related assets in the broader Purtera stack (loadable as future
inputs without changing this module's signature):

  * ``purpulse.app/SOWSmith_policy_pack_v2.yaml`` (~12k lines) — the
    canonical clause modules + service-line requiredness logic. The
    envelope's pre-computed cockpit fields (pm_dashboard,
  ...
  * ``purpulse.app/SOW_field_catalog_v2.yaml`` (~10.6k lines) — the
    full 707-field SOW Requirements Library. The envelope's SRL
    checklist currently covers a 41-field subset; the full catalog
    plugs in by extending the predicate framework.
...
Design principles:
  * Every section pulls FROM the envelope's pre-computed cockpit
    ...
    stakeholder_load, project_vitals). No re-deriving — these ARE
    the substrate's truth.
  ...
  * No LLM in the hot path. An LLM polish layer can soften phrasing,
    but the structural backbone + every receipt comes from this code.
"""
